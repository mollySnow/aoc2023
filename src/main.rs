...
fn calculate(digits: &[u32]) -> Option<u32> {
    let mut digits_iter = digits.iter();
    let first = digits_iter.next()?;

    if let Some(last) = digits_iter.next_back() {
        Some(first * 10 + last)
    } else {
        Some(first * 11)
    }
...
fn main() {
    let part1: u32 = INPUT.lines().filter_map(|line| calculate(&part1(line))).sum();
    let part2: u32 = INPUT.lines().filter_map(|line| calculate(&part2(line))).sum();
...
const TEST_ANSWER: u32 = 142;
const TEST_ANSWER_2: u32 = 281;

const TEST_3: &str = r#"5twofivedlk1pfjjmctjh
...
const INPUT: &str = r#"fivepqxlpninevh2xxsnsgg63pbvdnqptmg
