...
fn calculate(input: Vec<Hand>) -> u32 {
    ...
    vec.sort_by(|a, b| {
        let (str_a, _, str2_a, _) = a;
        let (str_b, _, str2_b, _) = b;
    ...
    vec.iter().fold((0, 1), |(sum, idx), (_, bid, _, _)| {
        (sum + bid * idx, idx + 1)
    }).0
}

fn part1(input: &str) -> u32 {
    calculate(parse_many(input, parse1).unwrap().1)
}

fn part2(input: &str) -> u32 {
    calculate(parse_many(input, parse2).unwrap().1)
}

fn parse_many(input: &str, parser: fn(&str) -> R<Hand>) -> R<Vec<Hand>> {
    separated_list1(multispace1, parser)(input)
}

const TRANSLATOR: [u32; 5] = [65536, 4096, 256, 16, 1];
fn parse1(input: &str) -> R<Hand> {
    ...
    for c in hand.chars() {
        ...
        strength2 += TRANSLATOR[current] * v; 
        current += 1;
        cards[idx] += 1;
    };
