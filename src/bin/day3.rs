...
const AdjacentPoints: [(isize, isize); 8] = [
    (0, 1),
    (1, 0),
    (1, 1),
    (1, -1),
...
#[derive(Debug, Clone)]
struct Symbol(char, (isize, isize));

impl Symbol {
    fn from_str(input: &str, predicate: fn(char) -> bool) -> Vec<Symbol> {
...
fn take_while(input: &str, predicate: fn(char) -> bool) -> (&str, &str) {
    let mut len = 0;
...
impl Number {
    fn from_str(input: &str) -> Vec<Number> {
        let mut result = Vec::new();
        let mut y = 0;
        ...
        for line in input.lines() {
            let mut x = 0;
            loop {
                let (slice, rest) = take_while(&line[x..], |c| !c.is_digit(10));
                x += slice.len();
                let (n, _) = take_while(rest, |c| c.is_digit(10));
                let start = (x as isize, y as isize);
                x += n.len();
                let end = ((x - 1) as isize, y as isize);
                let Ok(value) = n.parse::<u32>() else {
                    break;
                };
                result.push(Number {
                    value,
    ...
    fn is_adjacent_to_any(&self, symbols: &[Symbol]) -> bool {
        for symbol in symbols {
            let (val, (sx, sy)) = (symbol.0, symbol.1);
            for adjacent_point in AdjacentPoints.iter().map(|(x, y)| (x + sx, y + sy)) {
                if self.contains_point(adjacent_point) {
                    return true;
                }
            }
        }
    ...
    fn adjacent_symbols(&self, symbols_to_check: &[Symbol]) -> Vec<Symbol> {
        let mut result = Vec::new();
...
fn part1(input: &str) -> u32 {
    let numbers = Number::from_str(input);
    let symbols = Symbol::from_str(input, |c| !c.is_digit(10) && c != '.');

    numbers.iter().filter_map(|number| {
        if number.is_adjacent_to_any(&symbols) {
            Some(number.value)
        } else {
            None
        }
...
fn part2(input: &str) -> u32 {
    let numbers = Number::from_str(input);
    let symbols = Symbol::from_str(input, |c| c == '*');
    let mut m: HashMap<(isize, isize), HashSet<u32>> = HashMap::new();

    for number in numbers {
        for symbol in number.adjacent_symbols(&symbols) {
            let (x, y) = symbol.1;
            let entry = m.entry((x, y)).or_insert(HashSet::new());
            entry.insert(number.value);
        }
    }

    m.iter().filter_map(|(_, v)| {
        if v.len() == 2 {
...
const ANSWER_EXAMPLE_2: Option<u32> = Some(467835);
const EXAMPLE_2: &str = r#"467..114..
...*......
..35..633.
......#...
617*......
