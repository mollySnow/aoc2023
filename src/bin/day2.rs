...
struct Game {
    game_id: u32,
    sets: Vec<Vec<(ColorKind, u32)>>,
    current_set: Option<Vec<(ColorKind, u32)>>,
}
    ...
    fn is_valid(&self, game: &Game) -> bool {
        for set in &game.sets {
            ...
            for (color, count) in set {
                match color {
                    ColorKind::Red => red += count,
                    ColorKind::Green => green += count,
                    ColorKind::Blue => blue += count,
                }
            }
...
impl Game {
    ...
    fn add_to_set(&mut self, color: ColorKind, count: u32) {
        match &mut self.current_set {
            Some(set) => {
                set.push((color, count));
            },
            None => {
                self.current_set = Some(vec![(color, count)]);
            }
        }
    }
    fn complete_set(&mut self) {
...
fn parse_game(tokens: &[Token]) -> Vec<Game> {
    let mut index = 0;
    let len = tokens.len();
    ...
    loop {
        ...
        match &tokens[index..] {
            [Token::Game, Token::Number(id), Token::SymbolColon, ..] => {
                current_game = Some(Game::new(*id));
                index += 3;
            },
            [Token::Number(n), Token::Color(c), Token::SymbolComma, ..] => {
                if let Some(game) = current_game.as_mut() {
                    game.add_to_set(c.clone(), *n);
                    index += 3;
                }
            },
            [Token::Number(n), Token::Color(c), Token::SymbolSemicolon, ..] => {
                if let Some(game) = current_game.as_mut() {
                    game.add_to_set(c.clone(), *n);
                    game.complete_set();
                    index += 3;
            ...
            [Token::Number(n), Token::Color(c), Token::Newline, ..] => {
                if let Some(game) = current_game.as_mut() {
                    game.add_to_set(c.clone(), *n);
                    game.complete_set();
                    index += 3;
...
fn tokenize(input: &str) -> Vec<Token> {
    ...
    loop {
        ...
        if input[index..].starts_with("Game") {
            tokens.push(Token::Game);
            index += 4;
        } else if input[index..].starts_with("red") {
            tokens.push(Token::Color(ColorKind::Red));
            index += 3;
        } else if input[index..].starts_with("green") {
            tokens.push(Token::Color(ColorKind::Green));
            index += 5;
        } else if input[index..].starts_with("blue") {
            tokens.push(Token::Color(ColorKind::Blue));
            index += 4;
        } else if input[index..].starts_with(":") {
            tokens.push(Token::SymbolColon);
            index += 1;
        } else if input[index..].starts_with(",") {
            tokens.push(Token::SymbolComma);
            index += 1;
        } else if input[index..].starts_with(";") {
            tokens.push(Token::SymbolSemicolon);
            index += 1;
        } else if input[index..].starts_with("\n") {
            tokens.push(Token::Newline);
            index += 1;
        } else if input[index..].starts_with(char::is_whitespace) {
            // tokens.push(Token::Whitespace);
            index += 1;
        } else if input[index..].starts_with(char::is_numeric) {
            let mut number = 0;
            while index < len && input[index..].starts_with(char::is_numeric) {
                number = number * 10 + input[index..].chars().next().unwrap().to_digit(10).unwrap();
                index += 1;
            }
            tokens.push(Token::Number(number));
        } else {
            panic!("Unexpected token at index {}: {}", index, input[index..].chars().next().unwrap());
        }
    }
    tokens
}
...
fn main() {
    ...
    let rules = RuleSet::new(12, 13, 14);
    //part1
    //println!("tokens: {:?}", parse_game(&tokenize(PART_1_INPUT)).iter().filter_map(|g| if rules.is_valid(g) { Some(g.game_id) } else { None }).sum::<u32>());
    //part2
    println!("tokens: {:?}", parse_game(&tokenize(PART_1_INPUT)).iter().map(|g| g.get_power()).sum::<u32>());
}
...
const TEST_INPUT_ANSWER: u32 = 8;

const PART_1_INPUT_ANSWER: u32 = 2416;
const PART_1_INPUT: &str = r#"Game 1: 12 blue; 2 green, 13 blue, 19 red; 13 red, 3 green, 14 blue
